...
export default function SellerStockManagement() {
    ...
    const handleStockUpdate = async (item: StockItem) => {
        const draftValue = stockDrafts[item.variationId] ?? String(item.stock);
        ...
        setUpdatingStock(item.variationId);
        setSavedStock(null);
        try {
            const response = await updateStock(item.productId, item.variationId, newStock);
            if (response.success) {
                // Update local state
                setStockItems(prev => prev.map(stockItem =>
                    stockItem.variationId === item.variationId
                        ? { ...stockItem, stock: newStock }
                        : stockItem
                ));
                setStockDrafts(prev => {
                    const next = { ...prev };
                    delete next[item.variationId];
                    return next;
                });
                setSavedStock(item.variationId);
                window.setTimeout(() => {
                                    ...
                                    <td className="p-4 align-middle border border-neutral-200">
                                        <div className="flex items-center gap-2">
                                            <input
                                                ...
                                                step="1"
                                                inputMode="numeric"
                                                value={stockDrafts[item.variationId] ?? String(item.stock)}
                                                onChange={(e) => {
                                                ...
                                                className="w-24 px-3 py-2 border border-neutral-300 rounded-lg text-sm font-medium focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none"
                                                aria-label={`Stock for ${item.name} - ${item.variation}`}
                                                onKeyDown={(e) => {
                                                    if (e.key === 'Enter') {
                                            ...
                                            <button
                                                ...
                                                className="min-w-[72px] px-3 py-2 bg-teal-600 text-white rounded-lg text-xs font-semibold hover:bg-teal-700 transition-colors disabled:bg-neutral-300"
                                                title="Update Stock"
                                            >
