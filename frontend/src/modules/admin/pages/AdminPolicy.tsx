...
export default function AdminPolicy() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (response && response.success && response.data) {
        ...
        if (response.data.appPolicies) {
          setPolicies({
            customer: response.data.appPolicies.customer || '',
            seller: response.data.appPolicies.seller || '',
            delivery: response.data.appPolicies.delivery || ''
          });
        }
      }
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      showToast(error.response?.data?.message || 'Error updating policies', 'error');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white px-4 sm:px-6 py-4 border-b border-neutral-200">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-neutral-900">App Policies</h1>
          </div>
          <div className="text-sm text-neutral-600">
            <span className="text-blue-600">Home</span> / <span className="text-neutral-900">App Policies</span>
          </div>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm border border-neutral-200 overflow-hidden">
              <div className="bg-teal-600 px-4 sm:px-6 py-3">
                <h2 className="text-white text-lg font-semibold capitalize">{activeTab} App Policy Content</h2>
              </div>
              <div className="p-4 sm:p-6">
                <div>
                  <textarea
                    ...
                    placeholder={`Enter ${activeTab} App Policy content...`}
                    rows={20}
            ...
            <div className="flex justify-end gap-4">
              <button
                type="button"
                onClick={() => setPolicies({ ...policies, [activeTab]: '' })}
